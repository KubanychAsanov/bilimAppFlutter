import 'package:bilim_app/data/repository/i_auth_facade.dart';
...
class LoginFormBloc extends Bloc<LoginFormEvent, LoginFormState> {
  ...
  LoginFormBloc(this.authFacade) : super(LoginFormInitial()) {
    on<EmailChanged>((event, emit) {
      emit(LoginFormData(email: event.email));
    });
    ...
    on<LoginButtonPressed>((event, emit) async {
      ...
      if (data is Error) {
        emit(
          LoginFormData(
              isSubmitting: false, showErrors: true, errors: [data.error]),
        );
      }

      emit(LoginFormData(name: (data as Success).data));
    });
  }
}
